...
*c_diarrhea_pro	The treatment was provided by a formal provider (all public provider except other public, pharmacy, and private sector)
       /*please cross check as there might be case where the diarreha treatment provider is not in h12a-h12x*/
        ...
        replace `var' = . if ///
	    (regexm("`lab'","( other|shop|Shop|store|Store|chemist/ PMS|private dispensary|chemist/ pms|hsa|oth.priv sect|cs med.priv sector|mobile seller|Dumba Nengue|Market|pharmacy|market|Pharmacy|at home|kiosk|hakim|DAI-TBA|Traditional Practitioner|dispenser/compounder|Street|other public -|other private -|other -|Diarrhea: CS public sector|Diarrhea: CS private medical|Traditional practitioner|Relatives|diarrhea: cs private medical|volunteer|merchant|relative|Other|friend|church|Church|drug|addo|rescuer|trad|unqualified|stand|cabinet|ayush|^na|-na|na-|NA-|NA -|na -|na -|diarrhea: cs public sector)") ///
	    & !regexm("`lab'","(ngo|hospital|medical center|worker|women|bhu/fwc|maternity house|mchfp center|pvt clinic|pvt nurse/midwife|cabinet|rescuer|national|(fap/dac/ph)|gov. family planning center/cabi|private policlinic/ woman's co|private family planning center|policlinic/ woman's consultation|cnss|NGO med.|health stand|moving|cs other public facility)")) & !regexm("`lab'", "( lay)")
...
if ~inlist(name,"Bolivia2008") {		
	    ...
	    local lab: variable label `var' 
        replace `var' = . if ///
		(regexm("`lab'","( other| oth |shop|pharmacy|Shop|store|private dispensary|chemist/pms|oth.priv sect|mobile sales|cs med.priv sect|hsa|Store|Dumba Nengue|chemist/ PMS|Market|pharmacy|market|Pharmacy|at home|kiosk|hakim|DAI - TBA|Traditional Practitioner|dispenser/compounder|Street|other public -|other private -|other -|Fever/cough: CS public sector|Fever/cough: CS private medical|Traditional practitioner|Relatives|fever/cough: cs private medical|volunteer|merchant|market|kiosk|relative|friend|Other|church|drug|addo|rescuer|trad|unqualified|stand|cabinet|ayush|^na|-na|na-|NA-|na -|NA -|fever/cough: cs public sector)") ///
	    & !regexm("`lab'","(ngo|hospital|medical center|worker|women|bhu/fwc|cabinet|rescuer|national|policlinic/ woman's consultat|(fap/dac/ph)|private family planning cente|private policlinic/ woman's c|gov. family planning center/c|private policlinic/ woman's co|cnss|NGO med.|health stand|moving|other public facility)")) & !regexm("`lab'", "( lay)") 
		replace `var' = . if !inlist(`var',0,1) 
	    }
	    /* do not consider formal if contain words in 
	    the first group but don't contain any words in the second group */
